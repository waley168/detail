...
# path to font
# font_path: Path = Path(__file__).parent / "jf-openhuninn.ttf"
...
def _build_invoice_information():
        ...
        table_001.add(Paragraph("Date", horizontal_alignment=Alignment.RIGHT))
        now = datetime.now()
        table_001.add(Paragraph("%d/%d/%d" % (now.day, now.month, now.year)))

        table_001.add(Paragraph("[City, State, ZIP Code]"))
        table_001.add(Paragraph("Invoice #", horizontal_alignment=Alignment.RIGHT))
        table_001.add(Paragraph("%d" % random.randint(1000, 10000)))

        table_001.add(Paragraph("[Phone]"))
        table_001.add(Paragraph("Due Date", horizontal_alignment=Alignment.RIGHT))
        table_001.add(Paragraph("%d/%d/%d" % (now.day, now.month, now.year)))
...
def _build_itemized_description_table(row):
        ...
        table_001.add(TableCell(Paragraph("Subtotal", horizontal_alignment=Alignment.RIGHT, ),
                                col_span=3, ))
        table_001.add(TableCell(Paragraph(row[0], horizontal_alignment=Alignment.RIGHT)))
        table_001.add(TableCell(Paragraph("Discounts", horizontal_alignment=Alignment.RIGHT, ),
                                col_span=3, ))
        table_001.add(TableCell(Paragraph("$ 177.00", horizontal_alignment=Alignment.RIGHT)))
        table_001.add(TableCell(Paragraph("Taxes", horizontal_alignment=Alignment.RIGHT),
                                col_span=3, ))
        table_001.add(TableCell(Paragraph("$ 100.30", horizontal_alignment=Alignment.RIGHT)))
        table_001.add(TableCell(Paragraph("Total", horizontal_alignment=Alignment.RIGHT),
                                col_span=3, ))
...
def topdf(row, varstr):
        ...
        page_layout.add(
                Paragraph(
                        f"Hello World!{row[4].encode('utf-8')}",
                        font="Helvetica",
        ...
        page_layout.add(_build_itemized_description_table(row))

        with open(Path(f"{row[0]}.pdf"), "wb") as pdf_file_handle:
